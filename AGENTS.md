...
- Follow-up fix: user reported editor hints still were not showing after simplification. Current implementation uses `ConditionInlayHintsProvider` in `src/conditionInlayHints.ts`; all condition hints are hidden when Moonraker is disconnected, and when connected, conditions are evaluated directly with Nunjucks and shown if evaluation succeeds.
...
- Revisit strict condition-hint validation later. Desired eventual behavior: do not show misleading `-> False` for unresolved variables, while still allowing defaults like `{% set state = params.STATE|default('normal') %}`. The initial strict resolver caused editor hints to disappear and has been bypassed for now.
